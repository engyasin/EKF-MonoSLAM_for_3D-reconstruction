...
def add_points(p,cams,i):
    ...
    cams.points[i+1].x = z[2]
    cams.points[i+1].y = -z[0]
    cams.points[i+1].z = -z[1]
    ...
    cams.points[i+3].x = x[2]
    cams.points[i+3].y = -x[0]
    cams.points[i+3].z = -x[1]
    ...
    cams.points[i+5].x = y[2]
    cams.points[i+5].y = -y[0]
    cams.points[i+5].z = -y[1]

    return cams

def add_cor_for_pose(pos,points=[]):
    ...
    #H = np.vstack((H,[0,0,0,1]))    
    a,b,c,w,x,y,z = pos
    H = np.array([[1-2*(y**2+z**2) , 2*(x*y-z*w)   ,  2*(x*z+y*w)  , a],
                  [ 2*(x*y+z*w)    ,1-2*(x**2+z**2),  2*(y*z-x*w)  , b],
                  [ 2*(x*z-y*w)    , 2*(y*z+x*w)   ,1-2*(x**2+y**2), c],
                  [       0        ,    0          ,      0        , 1]])
...
while not rospy.is_shutdown():
    #TODO: points outside while
    #Cameras.points =[]
    for i,p in enumerate(all_vec_pose[0:-1:1]):
        #postam = PoseStamped()
        #Cameras = add_points(Cameras,p,i)
        points = add_cor_for_pose(p,points)
