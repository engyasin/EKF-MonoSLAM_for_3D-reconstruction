...
class VSlamFilter {
    ...
    cv::Mat frame, old_frame,lastOkImg;
	std::vector<int> lastImgPatches;
    cv::Mat originalImg;
    cv::Mat drawedImg;
    // drawedImg for what?
...
public:
    ...
    void predict(Vector3f Translation_Speed_Control = Vector3f::Zero(), Vector3f Rotational_Speed_Control = Vector3f::Zero(),bool Vcontrol = false);
    ...
    float Covariance_Parameter();
    //MatrixXf getSigma(int rows_r,int cols_r, int s_row ,int s_col);
    bool seedWithFirstImg(cv::Mat img,std::vector<int> Pnts, std::vector<cv::Point2f> FImgCorners);
    void rts_epoch(VectorXf &MU, MatrixXf &SIGMA,VectorXf MU_S,MatrixXf SIGMA_S,Vector3f dTspeed,Vector3f dRspeed ,double deltaT);

    cv::Point2f returnCentrPatchIndx(int i){ return patches[i].center;}
    void loopUpdate();
    double getDt();
    //cv::Ptr<cv::ORB> akaze;
    int noise_cov_factor;
};
