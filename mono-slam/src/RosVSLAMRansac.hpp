...
public:
	...
	void predict(Vector3f delta_Translation_Speed = Vector3f::Zero(), Vector3f delta_Rotational_Speed = Vector3f::Zero(),bool Vcontrol = false);
	void update(float v_x = 0, float w_z = 0);
	nav_msgs::Path getCameraPath();
	nav_msgs::Odometry getVisualOdometry(ros::Time stamp);
	MatrixXf getPointsFeatures();
};
