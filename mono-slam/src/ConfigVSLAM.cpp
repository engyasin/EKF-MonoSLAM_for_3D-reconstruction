...
ConfigVSLAM::ConfigVSLAM(char *file) {
	...
	if (file) {
		...
		//std::cout << "u0: "<<camParams.u0 << " and "<<flag << std::endl;
		camParams.u0 = camParams.u0/scale;
		cout << setw(20) << "camera.u0" <<  setw(30) << camParams.u0 << setw(30) << (flag ? "Read" : "Default(hard)") << endl;
		camParams.v0 = 204;
		...
		cout << setw(20) << "camera.v0" <<  setw(30) << camParams.v0 << setw(30) << (flag ? "Read" : "Default(hard)") << endl;
